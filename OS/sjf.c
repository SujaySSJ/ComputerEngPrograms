...
Process p[20],temp;
int schedule[200],idle=0,ongoing_process=0;
...
void sjf_scheduler(Process rp[20],int rno,int n){
	int i;
	temp = rp[0];
	...
	else temp=p[ongoing_process-1];	
	schedule[CPU_time++]=temp.id;
	for(i=0;i<n;i++){
		if(temp.id==p[i].id){
			p[i].rt--;
			ongoing_process=p[i].id;
			if(p[i].rt==0){
				p[i].flag=1;
				p[i].end=CPU_time;
